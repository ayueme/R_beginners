...
:::{.callout-note}
安装R包时一定要注意，R包的名字不能写错，一定要注意大小写！！双引号或者单引号都可以，但是必须是英文状态下的！！
:::
...
本地安装需要注意**R包依赖**的问题。R包依赖的意思是有些R包是建立在其他R包的基础上的，所以你在安装时需要注意先后顺序，必须先安装某个包然后才能安装另一个包，否则就会出现安装失败。比如`easyTCGA`就是建立在很多R包之上，所以如果你没提前安装`easyTCGA`的依赖包，那么也会报错。
